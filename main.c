...
void TI_setup() {
  ...
  A[0]=0x41;//set AINN to central voltage
  A[1]=0x00;
  A[2]=0x02;
  nrf_drv_spi_transfer(&spi,A,3,rx,3);
...
void TI_read(uint8_t data[5]) {
  //RDATA 0x01 0x02
  uint8_t tx[5] = {0x01,0x02,0,0,0};
  //uint8_t rx[5] = {0,0,0,0,0};
  nrf_drv_spi_transfer(&spi,tx,5,data,5);
...
void MAX_setup() {
  ...
  const uint8_t B[4] = {0x08,0x40,0x03,0x63};
...
void MAX_read_SPO2(uint8_t data[6]) {
  const uint8_t A[1] = {0x07};
  check_twi(nrf_drv_twi_tx(&twi,MAX30102_ADDR, A, sizeof(A),true));
  check_twi(nrf_drv_twi_rx(&twi,MAX30102_ADDR,data,6));
	data[0]&=3;
	data[3]&=3;  
	return;
}
int MAX_get_avail_samps() {
  uint8_t data[3];
  uint8_t A[1] = {0x00};
  check_twi(nrf_drv_twi_tx(&twi,MAX30102_ADDR, A, sizeof(A),true));
  check_twi(nrf_drv_twi_rx(&twi,MAX30102_ADDR,data,1));
	if(!(data[0]&0x40)) {
		return 0;
	}
  A[0] = 0x04;
  check_twi(nrf_drv_twi_tx(&twi,MAX30102_ADDR, A, sizeof(A),true));
  check_twi(nrf_drv_twi_rx(&twi,MAX30102_ADDR,data,3));
	if((data[1]&0x1F)!=0) {
		return 31;
  }
	if((data[2]-data[0])>=0) {
		return data[0]+(32-data[2]);//double check!!!
	} else {
		return data[0]-data[2]-1;//PROBABLY BROKE
	}
}

uint16_t MAX_read_temp() {
  uint8_t data[3];
  uint8_t A[2] = {0x21,0x01};//enable temp
  check_twi(nrf_drv_twi_tx(&twi, MAX30102_ADDR, A, sizeof(A),false));
  uint8_t B[2] = {0x03,0x02};
  check_twi(nrf_drv_twi_tx(&twi, MAX30102_ADDR, B, sizeof(B),false));
  uint8_t C[1] = {0x01};
  while(1) {
    check_twi(nrf_drv_twi_tx(&twi,MAX30102_ADDR, C, sizeof(C),true));
    check_twi(nrf_drv_twi_rx(&twi,MAX30102_ADDR,data,1));
    if(data[0]==2) {
      break;
    }
  ...
  uint8_t D[1] = {0x1F};
  check_twi(nrf_drv_twi_tx(&twi,MAX30102_ADDR, D, sizeof(D),true));
  check_twi(nrf_drv_twi_rx(&twi,MAX30102_ADDR,data,2));
  //data[1]&=0x0F;
  uint16_t ret_val = (data[0]<<8) + data[1];
	data[0]=0xC0;
	data[1]=0x00;
  check_twi(nrf_drv_twi_tx(&twi,MAX30102_ADDR,data,2,false));
	return ret_val;
}
...
int main(void)
{
  ...
  nrf_gpio_cfg_input(30,NRF_GPIO_PIN_PULLUP);//INT for MAX
  ...
  int num_samps=0;
  uint8_t TI_dat[30];
  uint8_t TI_outarr[18];
  uint8_t MAX_outarr[19];
  int TI_count = 0;
  int MAX_count = 0;
  uint16_t MAX_temp;
  uint16_t MAX_t_len = 2;
  //uint8_t here[5] = "here";
  while(true) {    
    //(pin_set), (pin_clear, pin_toggle)
      if(!nrf_gpio_pin_read(7)) {
      TI_read(TI_dat+5*TI_count);
      TI_count++;
      if(TI_count == 6) {
        for(int i = 0; i < 6; i++) {
          TI_outarr[3*i]=TI_dat[5*i];
          TI_outarr[3*i+1]=TI_dat[5*i+1];
          TI_outarr[3*i+2]=TI_dat[5*i+2];
        }
        ble_nus_data_send(&m_nus,TI_outarr,&TI_len,m_conn_handle);
        TI_count=0;
        MAX_temp = MAX_read_temp();
        ble_nus_data_send(&m_nus,(uint8_t*)&MAX_temp,&MAX_t_len,m_conn_handle);
      }
    }
    
    num_samps = MAX_get_avail_samps();
    for(int i = 0; i < num_samps; i++) {
      MAX_read_SPO2(MAX_outarr+6*MAX_count);
      MAX_count++;
      if(MAX_count==3) {
        ble_nus_data_send(&m_nus,MAX_outarr,&MAX_len,m_conn_handle);
        MAX_count=0;
        //nrfx_uart_tx(&uart,here,5);
      }
    }
  }
}
